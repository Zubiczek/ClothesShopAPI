...
namespace ClothesShopAPI.Services.EmailService
{
    public class ResetPasswordEmailService : IEmailService
    {
        ...
        public ResetPasswordEmailService(IMailService mailService,
        ...
        public async Task SendEmail(UserEntity user, string token)
        {
            var confirmationlink = _urlHelper.Action
                ("ResetPassword", "User", new { userId = user.Id, token = token }, _request.Scheme);
            ...
            "<div style = 'padding-top: 30px; text-align: center;'>" +
                "Witaj, Reset hasła prawie gotowy!" +
            "</div>" +
            "<div style = 'padding-top: 30px; text-align: center;'>" +
